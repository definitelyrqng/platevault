...
export default function BelgiumUploadPage() {
  ...
  async function doUpload() {
    ...
    try {
      ...
      setStatus("done");
      setTimeout(() => router.push("/c/belgium"), 1500);
    } catch (err) {
      setStatus("error");
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
          <form onSubmit={handleSubmit} className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-6 space-y-5">
            ...
            <div className="grid gap-4">
              {/* Category — with auto-detect note */}
              <label className="grid gap-1.5">
                ...
                <select value={category} onChange={(e) => handleCategoryChange(e.target.value as BelgiumCategoryId)}
                  ...
                  {BELGIUM_CATEGORIES.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.label}{c.color === "green" ? "  (green plate)" : ""}
                    </option>
                  ))}
                </select>
              ...
              {/* Plate input — smart single field */}
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              {status === "idle" && (
                <span className="text-xs text-zinc-500">
                  {!file ? "Add a photo"
                    : plateText.trim().length < 1 ? "Enter plate text"
                    : location.trim().length < 2 ? "Enter a location"
                    : "Ready ✓"}
                </span>
              )}
            </div>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 p-5">
              ...
              <p className="text-sm text-zinc-400 leading-relaxed">
                City + country required — e.g. <span className="text-zinc-300">Brussels, Belgium</span> or <span className="text-zinc-300">Ghent, Belgium</span>.
              </p>
            </div>
