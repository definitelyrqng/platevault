...
function locationTooExact(loc: string) {
  ...
  const words = ["ulitsa", "street", "str.", "road", "avenue", "blvd", "bul."];
  return words.some((w) => t.includes(w)) && /\b\d{1,4}[a-z]?\b/.test(t);
}

export default function BulgariaUploadPage() {
  const router = useRouter();

  const [category, setCategory] = useState<BulgariacategoryId>("standard");
  const [plateText, setPlateText] = useState("");
  const [format, setFormat] = useState<string>(BULGARIA_FORMATS_FOR["standard"][0]);

  function handleCategoryChange(c: BulgariacategoryId) {
    setCategory(c);
    setPlateText("");
    setFormat(BULGARIA_FORMATS_FOR[c][0]);
  }
  ...
  const catMeta = BULGARIA_CATEGORIES.find((c) => c.id === category);
  const availableFormats = BULGARIA_FORMATS_FOR[category] ?? [];
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const r = await fetch(
        `/api/uploads/check?plateText=${encodeURIComponent(plateText.trim())}&country=bulgaria`
      );
      if (r.ok) {
        ...
        <div className="mx-auto max-w-5xl">
          ...
          {status === "done" && (
            <div className="mt-6 rounded-2xl border border-emerald-800 bg-emerald-950/40 px-5 py-4 text-sm text-emerald-300">
              Uploaded! Redirecting to the Bulgaria gallery…
            </div>
          )}
          {status === "error" && (
          ...
          {multiSpotWarning && (
            <div ref={warningRef} className="mt-6 rounded-2xl border border-amber-800 bg-amber-950/30 px-5 py-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => setMultiSpotWarning(null)}
                  className="flex-1 rounded-xl border border-zinc-700 bg-zinc-900 py-2 text-sm font-medium text-zinc-300 hover:bg-zinc-800 transition-colors">
                  Cancel
                </button>
                <button onClick={doUpload}
                  className="flex-1 rounded-xl border border-amber-800 bg-amber-950/60 py-2 text-sm font-medium text-amber-300 hover:bg-amber-950 transition-colors">
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5 space-y-3">
                <div className="text-sm font-medium text-zinc-200">Plate category</div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {BULGARIA_CATEGORIES.map((c) => (
              ...
              <div className="rounded-xl border border-zinc-800/60 bg-zinc-900/20 px-4 py-3 text-sm text-zinc-400 leading-relaxed">
                <strong className="text-zinc-200">{catMeta?.label}</strong>
                {catMeta?.example && (
                  <span className="ml-2 text-indigo-400">· e.g. {catMeta.example}</span>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-6 space-y-5">
                ...
                <div>
                  <span className="block text-sm text-zinc-300 mb-2">
                    Photo <span className="text-zinc-600 text-xs">(JPG or PNG, max 8 MB)</span>
                  </span>
                  <label
                ...
                <div className="grid gap-1.5">
                  ...
                  <BulgariaPlateInput
                    ...
                    onCategoryDetected={(c) => {
                      if (c !== category) handleCategoryChange(c);
                    }}
                  />
                </div>

                {/* Format (only for multi-format categories) */}
                {availableFormats.length > 1 && (
                  <div className="grid gap-1.5">
                    <span className="text-sm text-zinc-300">Plate layout</span>
                ...
                <label className="grid gap-1.5">
                  <span className="text-sm text-zinc-300">
                    Location <span className="text-zinc-600 text-xs">(city + country required)</span>
                  </span>
                  <input
                    value={location}
            ...
            <aside className="space-y-4">
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 p-5">
                ...
                <p className="text-xs text-indigo-400 font-mono">{format}</p>
                <p className="mt-1 text-xs text-zinc-500 leading-relaxed">{BG_FORMAT_LABELS[format] ?? format}</p>
              </div>

              {/* Format guide */}
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 p-5">
                <div className="text-xs uppercase tracking-wider text-zinc-500 mb-3">Plate formats</div>
                <div className="space-y-2">
                  {BULGARIA_CATEGORIES.map((c) => (
                    <div key={c.id}
                      className={`flex justify-between items-center rounded-xl px-3 py-2 text-xs transition-colors ${
                        c.id === category ? "bg-zinc-800 text-zinc-100" : "text-zinc-500"
                      }`}>
                      <span>{c.label}</span>
              ...
              {/* Region guide */}
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 p-5">
                <div className="text-xs uppercase tracking-wider text-zinc-500 mb-3">Region codes</div>
                <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
                  {[
                    ["A","Burgas"], ["B","Varna"], ["BH","Vidin"], ["BP","Vratsa"],
                    ["BT","Veliko Tarnovo"], ["C/CA/CB","Sofia City"], ["CC","Silistra"],
                    ["CH","Sliven"], ["CM","Smolyan"], ["CO","Sofia Province"],
                    ["CT","Stara Zagora"], ["E","Blagoevgrad"], ["EA","EV (Sofia)"],
                    ["EB","Gabrovo"], ["EH","Pleven"], ["EM","E-Moto"],
                    ["H","Shumen"], ["K","Kardzhali"], ["KH","Kyustendil"],
                    ["M","Montana"], ["OB","Lovech"], ["P","Ruse"],
                    ["PA","Pazardzhik"], ["PB","Plovdiv"], ["PK","Pernik"],
                    ["PP","Razgrad"], ["T","Targovishte"], ["TX","Dobrich"],
                    ["X","Khaskovo"], ["Y","Yambol"],
                  ].map(([code, name]) => (
                    <div key={code} className="flex justify-between text-[11px] text-zinc-500">
                      <span className="font-mono font-semibold text-zinc-300">{code}</span>
                      <span>{name}</span>
                    </div>
                  ))}
                </div>
