...
export default function AustriaUploadPage() {
  ...
  async function doUpload() {
    ...
    try {
      ...
      setStatus("done");
      setTimeout(() => router.push("/c/austria"), 1500);
    } catch (err) {
      setStatus("error");
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100 px-4 py-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">Photo, plate text, city. Done in seconds.</p>
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
          ...
          <form onSubmit={handleSubmit} className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-6 space-y-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="grid gap-1.5 md:col-span-2">
                ...
                <select
                  value={category}
                  onChange={(e) => setCategory(e.target.value as AustriaCategoryId)}
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/30 p-5">
              ...
              {regionLabel && (
                <p className="mt-3 text-xs text-blue-400 text-center">{regionLabel}, Austria</p>
              )}
            </div>

            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 p-5">
              <div className="text-sm font-medium text-zinc-200 mb-2">&#x1F4CB; Plate type</div>
              <p className="text-sm text-zinc-400 leading-relaxed">{plateTypeLabel}</p>
            </div>

            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 p-5">
              <div className="text-sm font-medium text-zinc-200 mb-2">&#x1F4CD; Location rule</div>
              <p className="text-sm text-zinc-400 leading-relaxed">
                City is required, e.g. <span className="text-zinc-300">Vienna, Austria</span>. No street names or house numbers.
              </p>
            </div>
          </aside>
        </div>
